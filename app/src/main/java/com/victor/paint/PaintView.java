...
public class PaintView extends View {
    ...
    public void setInputString(String text, String size, Boolean isBold, Boolean isItalic, Boolean LineType) {
        ...
        if (isBold & isItalic)
            paintText.setTypeface(Typeface.create(Typeface.DEFAULT, Typeface.BOLD_ITALIC));
        ...
        if (actionIndex == 0) {
            ...
            switch (action) {
                ...
                case MotionEvent.ACTION_MOVE:
                    ...
                    if (isShift) moveFigure(x, y); // Режим сдвиг
    ...
    private void touchStarted(float x, float y, int lineID) {
        ...
        MyPath path; // used to store the path for the given touch id
    ...
    private void touchEnded(int lineID) {
        ...
        if (pathStack.size() >= Stacksize) {
            //  OverStack(pathStack);  //  перевернуть стек

            MyPath path = new MyPath();
            if (overStack(pathStack)) {
                path = (MyPath) pathStack.pop(); // Извлечь path из стека
                Log.d("Push", "P вышел из стека");
            } else Log.d("Overstek", " Стек пуст");

            overStack(pathStack);  //  перевернуть стек
            bitmapCanvas.drawPath(path.getTransformPath(), path.paint);// draw path to bitmapCanvas
        }

        switch (selected_tool) {
            case "pencil":
                MyPath path = pathMap.get(lineID); // get the corresponding Path
                MyPath npath = new MyPath(path);        // скопировать path
                npath.rectf.setEmpty();
                npath.setInrotable();
                pathStack.push(npath);
                path.setPaint(paintLine);

                Log.d("Push", "P1 зашел в стек");// занести в стек
                path.reset(); // reset the Path
                break;
            case "eraser":
                path = pathMap.get(lineID); // get the corresponding Path
                npath = new MyPath(path); // скопировать path
                npath.setInrotable();
                npath.rectf.setEmpty();
            ...
            case "copy":
                ...
                npath = new MyPath(dPath);    // скопировать dPath
                pathStack.push(npath);  // занести в стек
